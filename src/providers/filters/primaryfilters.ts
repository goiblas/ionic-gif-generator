...
// No adjustment
export const invert = (pixels, adj) => {
 let d = pixels.data;
 for (let i = 0; i < d.length; i += 4) {
   d[i] = 255 - d[i];
   d[i + 1] = 255 - d[i + 1];
...
export const brightness = (pixels, adj) => {
  let d = pixels.data;
  adj = (adj > 1) ? 1 : adj;
  adj = (adj < -1) ? -1 : adj;
  adj = ~~(255 * adj);
...
export const hueSaturation = (pixels, adj) => {
  let d = pixels.data;
  for (let i = 0; i < d.length; i += 4) {
...
export const saturation = (pixels, adj) => {
  let d = pixels.data;
  adj = (adj < -1) ? -1 : adj;
  for (let i = 0; i < d.length; i += 4) {
    let r = d[i], g = d[i + 1], b = d[i + 2];
...
export const contrast = (pixels, adj) => {
  adj *= 255;
  let d = pixels.data;
...
export const colorFilter = (pixels, rgbColor) => {
  let d = pixels.data;
  let adj = rgbColor[3];
  for (let i = 0; i < d.length; i += 4) {
...
// RGB Adjust
export const rgbAdjust = (pixels, rgbAdj) => {
  let d = pixels.data;
...
export const convolute = (pixels, weights) => {
